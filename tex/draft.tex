...
\section{Long-Lived Particles}
    ...
    On the other hand, models with weakly coupled R-parity violating (RPV) terms produce long-lived but not perfectly stable LSPs. Fig.~\ref{rp_and_rpv_diagrams} shows two possible processes, one where R-parity gaurantees a stable LSP and one where RPV produces a long-lived but stable LSP. A similar situation arises in GMSB models\fxnote{should I describe GMSB?} where the gravitino is the LSP. The coupling between the next-to-LSP (NLSP) and gravitino is inversely proportional to the scale of SUSY breaking, which supresses the NSLP decay rate and can lead to LLPs.\fxnote{cite dimopoulos low energy}

    \noindent \begin{figure}[htbp] \begin{center}
    \includegraphics[width=0.3\textwidth]{images/rp_and_rpv_diagrams}
...
\section{LHC Searches for Long-lived Particles}
    ...
    \subsection{Disappearing Tracks}
        ...
        \end{center} \end{subfigure}
        \begin{subfigure}[htbp]{0.3\textwidth} \begin{center}
        \includegraphics[width=\textwidth]{images/disappearing_strong_200ps_limit}
        \end{center} \end{subfigure}
        \begin{subfigure}[htbp]{0.3\textwidth} \begin{center}
        \includegraphics[width=\textwidth]{images/disappearing_strong_1ns_limit}
        \end{center} \end{subfigure}
        \caption{Limits.}
